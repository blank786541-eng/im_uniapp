...
declare interface Client {
  ...
  publish(
    stream: Stream,
    mediaType?: 'video' | 'audio' | 'screen' | 'audioSlave' | 'all'
  ): Promise<undefined>
  /**
   * 停止将本地音视频流发布到本房间。
   *
   ...
   *
   */
  getUid(): string | number | undefined
  ...
  /**
   * 开启实时字幕，并设置源语言和目标语言。
   * @param sourceLan 源语言，支持语言列表请参考开发文档，支持'AUTO'，表示自动检测语言。
   * @param targetLan 目标语言，支持语言列表请参考开发文档，不填或者填空字符串表示不翻译为其他语言。
   ...
   * // 开启实时字幕
   * client.startAsrCaptions('AUTO', '')
   * ```
   */
  startAsrCaptions(sourceLan: string, targetLan?: string): boolean
  ...
  /**
   * 指定编解码类型
   * @param type 'VP8' | 'H264'，可不填，不填写时根据房间能力自动选择编解码类型
   */
   ...
   * @param uid 机器人UID
   */
  aiManualInterrupt(uid: string): void

  /**
   * 用于发起MPS通知
   * @since V5.8.21
   */
  setMpsNotify(action: number, options: any): void

  /**
   * 注册AI音效插件
   * @since V5.8.0
   * @param {pluginOptions} options 插件参数说明
   * @param {String} options.key 插件标识， 'AIAudioEffects'
   ...
   *  await client.registerPlugin(options)
   * ```
   *
   */
   ...
   * @example
   * ```Javascript
   *  await client.unregisterPlugin('AIAudioEffects')
   * ```
   *
   */
   ...
   * @since V5.8.0
   *
   * 注意：该方法仅对开启混音的远端音频流生效。
   * @example
   * ```Javascript
   * await client.enableAudioEffect()
   * ```
   */
   ...
   * @example
   * ```Javascript
   * await client.disableAudioEffect()
   * ```
   */
   ...
   * @example
   * ```Javascript
   * client.setAudioEffectLite('Pitch', 0)
   * ```
   */
  ...
  /**
   * `uid-duplicate` 事件表示当前有人使用相同的uid加入了房间，你被踢出了。
   */
  ...
  on(
    event: 'custom-data',
    callback: (evt: { uid: string | number; customData: string }) => void
  ): void
  ...
  on(
    event: 'TrackEnded',
    callback: (mediaType: 'video' | 'audio' | 'screen' | 'screenAudio') => void
  ): void
   ...
   * ```
   * @note
   * - v5.8.25版本开始，safari浏览器，在首次授予浏览器设备采集权限后，会触发该回调。
   * - pc端：Firefox不支持设备检测
   */
  on(
   ...
   *
   * @note
   * - safari 浏览器（mac系统，v18.4版本才开始支持）
   * - 移动端浏览器不支持
   * - pc端：Firefox不支持设备检测
   * - v5.8.25版本开始，safari浏览器（>= v18.4版本），在首次授予浏览器设备采集权限后，会触发该回调。
   *
   */
  on(
  ...
  /*
   * `abortError` 事件由于系统异常，无法链接到媒体设备，建议重启系统。
   */
  on(event: 'abortError', callback: (mediaType: MediaType) => void): void

  /*
   * `unknownError` 事件表示由于未知原因，无法链接到媒体设备。
   */
  on(event: 'unknownError', callback: (mediaType: MediaType) => void): void

  /**
   *`auto-mask-end` 自动打码取消的通知
   * @param uid 表示当前取消打码的用户id
   * @param maskStreamType 表示当前打码的流类型，video或screen
   * @param maskArea 表示当前打码的区域，1为区域打码，0为全局打码
   * @param hitLocationInfos 表示当前打码的区域位置信息
   */
  on(
    event: 'auto-mask',
    callback: (evt: {
      uid: string
      maskStreamType: 'video' | 'screen'
      maskArea: 1 | 0
      hitLocationInfos: HitLocationInfos
    }) => void
  ): void
   ...
   *`auto-mask-end` 自动打码取消的通知
   * @param uid 表示当前取消打码的用户id
   * @param maskStreamType 表示当前取消打码的流类型，video或screen
   */
  on(
    event: 'auto-mask-end',
    callback: (evt: { uid: string; maskStreamType: 'video' | 'screen' }) => void
  ): void
   ...
   *
   */
  on(event: 'plugin-load', callback: (type: 'AIAudioEffects') => void): void
  ...
  on(
    ...
    callback: (evt: { type: 'AIAudioEffects'; msg: string }) => void
  ): void
  ...
  on(
    ...
    callback: (evt: { key: 'AIAudioEffects'; msg: string }) => void
  ): void
  ...
  on(
    event: 'ai-data',
    callback: (evt: { type: string; data: any; uid: number | string }) => void
  ): void
