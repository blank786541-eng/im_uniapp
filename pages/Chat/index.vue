...
import {TeamStore} from "@xkit-yx/im-store-v2/dist/types/teams";
...
const teamWatch = autorun(() => {
  if (to) {
    team.value = uni.$UIKitStore.teamStore.teams.get(to)
    console.log(team.value, 'team value====')
    // teamMembers.value = uni.$UIKitStore.teamMemberStore.getTeamMember(to)
  }
})
onMounted(() => {
  setNavTitle()
...
//
// function  close(){
//     uni.setStorageSync('closeNotice',true)
